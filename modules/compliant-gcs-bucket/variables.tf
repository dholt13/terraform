...
variable "gcp_project" {
  type        = string
  description = "GCP project ID is where the bucket and KMS resources will live."
}

variable "location" {
  type        = string
  description = "This is the GCS bucket location. Multi-regions like US, EU are valid for buckets."
  default     = "us-central1"
}

variable "kms_location" {
  type        = string
  description = "This is the KMS keyring location. Must be a single region (multi-regions are not supported for keyrings)."
  default     = "us-central1"
}
